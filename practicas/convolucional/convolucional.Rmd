...
library(formatR)
library(kableExtra)
knitr::opts_chunk$set(tidy.opts = list(width.cutoff = 60),echo = FALSE,tidy=TRUE, warning=FALSE)
```
...
## Ver si las diferencias son significativas
plot(density(val_acc),main="",xlab="Valores de exactitud de validación")

```
```{r}
plot(density(med_train), main="",xlab="Medias de exactitud de entrenamiento")
```
Empleo la prueba no paramétrica de los signos para 2 muestras, dado que ambas son muy asimétricas. \
...
```
```{r}
df_trainval <- cbind(minilotes,med_train,val_acc)
df_trainval <- df_trainval[1:50,]
knitr::kable(df_trainval, format = "markdown", digits = 6, 
             row.names = FALSE, align = c("l", "c", "r"))
```
