...
\
4.2\
Aplicada a MNIST (tm/5 (0.0002),50 épocas, 900 imágenes de training, 100 de test):\
Exactitud = 99 %\
Matriz de confusión:\

```{r}
confMat_MNIST_test <- read.csv("E:\\GDrive1\\Uni\\Master\\tecnologia_conocimiento\\practicas\\convolucional\\confMat_MNIST_test.csv", header=FALSE,sep=",")

knitr::kable(confMat_MNIST_test, format = "latex", digits = 2, 
             row.names = FALSE, align = c("l", "c", "r"), col.names = NULL)
```
Aplicada a base propia (tm/5 (0.0002),50 épocas, 900 imágenes de training, 100 de test):\
Exactitud =  25 %\
Matriz de confusión:\
```{r}
confMat_propia_test <- read.csv("E:\\GDrive1\\Uni\\Master\\tecnologia_conocimiento\\practicas\\convolucional\\confMat_propia_test.csv", header=FALSE,sep=",")

knitr::kable(confMat_propia_test, format = "latex", digits = 2, 
             row.names = FALSE, align = c("l", "c", "r"), col.names = NULL)
```
